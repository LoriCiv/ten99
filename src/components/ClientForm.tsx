...
interface ClientFormProps {
    ...
    onDuplicate?: () => void; // ✅ 1. Add the new prop for the duplicate handler
}
...
export default function ClientForm({ initialData, onSave, onCancel, isSubmitting, onDuplicate }: ClientFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-card p-6 rounded-lg shadow-lg border space-y-6">
            <h2 className="text-2xl font-bold mb-6 text-foreground">{isEditMode ? 'Edit Company' : 'New Company Details'}</h2>
            ...
            <div className="flex justify-end items-center gap-4 pt-4 border-t">
                {/* ✅ 2. Add the Duplicate button, which only shows in edit mode */}
