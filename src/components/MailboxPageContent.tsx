...
function MailboxPageContentInternal({ userId }: { userId: string }) {
    ...
    const [isActionLoading, setIsActionLoading] = useState(false); // For general actions
    const [isSendingMessage, setIsSendingMessage] = useState(false); // ✅ Dedicated state for the send button
    ...
    const filteredMessages = useMemo(() => {
        // ... (your existing filter logic is great, no changes needed)
        let items = messages;
        ...
        const sortedItems = items.sort((a,b) => (b.createdAt as Timestamp).toMillis() - (a.createdAt as Timestamp).toMillis());
        if (!searchTerm) return sortedItems;
        const lowercasedTerm = searchTerm.toLowerCase();
        return sortedItems.filter(message => (message.senderName?.toLowerCase() || '').includes(lowercasedTerm) || (message.subject?.toLowerCase() || '').includes(lowercasedTerm) || (message.body?.toLowerCase() || '').includes(lowercasedTerm));
    ...
    // ✅ REPAIRED: This function now uses its own dedicated loading state
    const handleSend = async (recipients: string[], subject: string, body: string) => {
        if (!userId || !userProfile) return false;
        setIsSendingMessage(true); // Use dedicated state
        try {
            ...
            showStatusMessage("error", `Failed to send message.`);
            return false;
        } finally {
            setIsSendingMessage(false); // Use dedicated state
        }
    };
    ...
    // ... (Your other action handlers like performAction, handleMarkAsEducation, etc. are great and don't need changes)
    const performAction = async (actionLogic: () => Promise<void>, successMessage: string) => { if (!selectedMessage?.id) return; setIsActionLoading(true); try { await actionLogic(); showStatusMessage("success", successMessage); setSelectedMessage(null); } catch (error) { console.error("Mailbox action failed:", error); showStatusMessage("error", `Action failed: ${error instanceof Error ? error.message : 'Unknown error'}`); } finally { setIsActionLoading(false); } };
    const handleMarkAsEducation = () => { if (!selectedMessage) return; if (selectedMessage.appointmentId) { performAction(() => markAsEducation(userId, selectedMessage), 'Event marked as education!'); } else { setEducationFormData({ date: '', time: '' }); setIsEducationModalOpen(true); } };
    const handleScheduleEducation = async () => { if (!selectedMessage || !educationFormData.date || !educationFormData.time) { showStatusMessage("error", "Please provide a date and time."); return; } const appointmentData: Partial<Appointment> = { subject: selectedMessage.subject, date: educationFormData.date, time: educationFormData.time, eventType: 'education', status: 'scheduled', notes: `Event scheduled from email:\nFrom: ${selectedMessage.senderName}\nSubject: ${selectedMessage.subject}\n\n${selectedMessage.body}` }; await performAction(() => createEducationAppointmentFromMessage(userId, selectedMessage, appointmentData), 'Education event scheduled!'); setIsEducationModalOpen(false); };
    ...
    return (
        <>
            ...
            <Modal isOpen={isEducationModalOpen} onClose={() => setIsEducationModalOpen(false)}><div className="p-6"><h2 className="text-xl font-bold mb-4">Schedule Event</h2><p className="text-sm text-muted-foreground mb-4">Please set the date and time for this workshop.</p><div className="space-y-4"><div><label htmlFor="educationDate" className="block text-sm font-medium">Date</label><input type="date" id="educationDate" value={educationFormData.date} onChange={(e) => setEducationFormData(prev => ({...prev, date: e.target.value}))} className="w-full mt-1 p-2 bg-background border rounded-md" /></div><div><label htmlFor="educationTime" className="block text-sm font-medium">Time</label><input type="time" id="educationTime" value={educationFormData.time} onChange={(e) => setEducationFormData(prev => ({...prev, time: e.target.value}))} className="w-full mt-1 p-2 bg-background border rounded-md" /></div></div><div className="flex justify-end gap-4 mt-6"><button type="button" onClick={() => setIsEducationModalOpen(false)} className="bg-secondary text-secondary-foreground font-semibold py-2 px-4 rounded-lg hover:bg-secondary/80">Cancel</button><button onClick={handleScheduleEducation} disabled={isActionLoading} className="bg-primary text-primary-foreground font-semibold py-2 px-4 rounded-lg flex items-center gap-2 disabled:opacity-50">{isActionLoading ? <Loader2 size={16} className="animate-spin"/> : <CalendarCheck size={16}/>}Schedule Event</button></div></div></Modal>
        </>
    );
}
