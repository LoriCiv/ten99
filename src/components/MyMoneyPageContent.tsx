...
import { 
    ArrowLeft, Trash2, Search, Check, X as XIcon, Send, RotateCcw, 
    Clock, CalendarCheck, CornerDownLeft, Pencil, PackageOpen, Loader2, 
    Inbox, SendHorizontal, CheckCircle, Hourglass, BookOpen, ThumbsUp, Info 
} from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
        <div className="bg-card rounded-lg shadow-xl w-full max-w-md border p-6 text-center">
            ...
            <div className="flex justify-center gap-4">
                ...
                <button onClick={onConfirm} className="bg-primary text-primary-foreground font-semibold py-2 px-4 rounded-lg hover:bg-primary/90">Confirm</button>
            </div>
        </div>
    </div>
);
...
function MailboxPageInternal({ userId }: { userId: string }) {
    ...
    const filteredMessages = useMemo(() => {
        let items = messages;
        if (activeFolder === 'inbox') { items = messages.filter(m => m.status === 'new' || m.status === undefined || m.type === 'inbound-offer'); } 
    ...
    const handleDelete = () => {
        if (!selectedMessage?.id || !userId) return;
        setConfirmation({
    ...
    if (!isFirebaseAuthenticated || isLoading) {
        return (
            <div className="flex justify-center items-center h-[calc(100vh-10rem)] p-8">
    ...
    const messageListComponent = (
        <div className="flex flex-col h-full border-r">
            ...
            <nav className="p-2 border-b">
                <FolderButton folderName="inbox" label="Inbox" icon={Inbox} count={messages.filter(m => !m.isRead && (m.status === 'new' || m.status === undefined || m.type === 'inbound-offer')).length} />
                ...
                {filteredMessages.length > 0 ? filteredMessages.map(message => (
                    <div key={message.id} onClick={() => handleSelectMessage(message)} className={`p-4 border-b cursor-pointer hover:bg-muted ${selectedMessage?.id === message.id ? 'bg-muted' : ''} ${!message.isRead && activeFolder === 'inbox' ? 'font-bold' : ''}`}>
                        <div className="flex justify-between text-sm">
                            <p className="truncate">{activeFolder === 'sent' ? `To: ${message.recipientId}` : message.senderName}</p>
