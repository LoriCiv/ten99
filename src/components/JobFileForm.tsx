...
export default function JobFileForm({
  ...
  // ✅ 1. Add state for the active notes tab
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (selectedFile) {
            ...
            finalFileUrl = await uploadFile(TEMP_USER_ID, selectedFile);
  ...
  return (
    <div className="bg-card p-6 rounded-lg shadow-lg border">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* ✅ 2. Add the tab navigation and conditional textareas */}
        <div>
        ...
        <div className="flex justify-end items-center mt-4 pt-4 border-t border-border">
          <div className="flex space-x-3">
            ...
            <button type="submit" disabled={isLoading} className="bg-primary text-primary-foreground font-semibold py-2 px-4 rounded-lg hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2 w-36 justify-center">
              {isUploading ? <><Loader2 size={16} className="animate-spin" /> Uploading...</> : (isSubmitting ? 'Saving...' : (isEditMode ? 'Update File' : 'Save File'))}
            </button>
          </div>
        </div>
      </form>
