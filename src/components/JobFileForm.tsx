...
interface JobFileFormProps {
    onSave: (data: Partial<JobFile>) => Promise<void>;
    onCancel: () => void;
    clients: Client[];
...
export default function JobFileForm({
    ...
    const [formState, setFormState] = useState<Partial<JobFile>>({});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let finalFileUrl = formState.fileUrl || '';

        try {
            if (selectedFile) {
                setIsUploading(true);
                finalFileUrl = await uploadFile(userId, selectedFile);
                setIsUploading(false);
            }

            const dataToSave = { ...formState, tags, fileUrl: finalFileUrl };
            await onSave(dataToSave);

        } catch (error) {
            console.error("Error saving job file:", error);
            alert("Failed to save job file. Check the console for details.");
            setIsUploading(false);
        }
    };
        ...
        <div className="bg-card p-6 rounded-lg shadow-lg border">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {formState.fileUrl && !selectedFile ? (
                        <div className="mt-2 flex items-center justify-between p-2 border rounded-md bg-background">
                            <a href={formState.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-primary hover:underline truncate">
                                <FileIcon size={16} />
                                <span className="truncate">{decodeURIComponent(formState.fileUrl.split('%2F').pop()?.split('?')[0] || "View Attached File")}</span>
                            </a>
                            <button type="button" onClick={() => { setSelectedFile(null); setFormState(prev => ({ ...prev, fileUrl: '' })); }} className="p-1 text-destructive hover:bg-destructive/10 rounded-full">
                                <Trash2 size={16} />
                            </button>
                        </div>
                    ) : (
                ...
                <div>
                    ...
                    <div className="mt-4">
                        {activeTab === 'shared' && (
                            <textarea name="sharedNotes" value={formState.sharedNotes || ''} onChange={handleInputChange} rows={6} className="w-full p-2 border rounded-md bg-background" placeholder="These notes will be visible on shared links..."></textarea>
                        )}
                        {activeTab === 'private' && (
                            <textarea name="privateNotes" value={formState.privateNotes || ''} onChange={handleInputChange} rows={6} className="w-full p-2 border rounded-md bg-background" placeholder="These notes are for your eyes only..."></textarea>
                        )}
                    </div>
                </div>
                ...
                <div className="flex justify-end items-center mt-4 pt-4 border-t border-border">
                    <div className="flex space-x-3">
                        ...
                        <button type="submit" disabled={isLoading} className="bg-primary text-primary-foreground font-semibold py-2 px-4 rounded-lg hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2 w-36 justify-center">
                            {isUploading ? <><Loader2 size={16} className="animate-spin" /> Uploading...</> : (isSubmitting ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : (isEditMode ? 'Update File' : 'Save File'))}
                        </button>
                    </div>
                </div>
            </form>
