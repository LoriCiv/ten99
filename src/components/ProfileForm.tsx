        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t">
                ...
                <div>
                    ...
                    <div className="space-y-4">{ (formData.education || []).map((entry, index) => (<div key={index} className="grid grid-cols-1 gap-2 p-3 border rounded-md bg-background/50 relative"><input value={entry.degree} onChange={e => handleEducationChange(index, 'degree', e.target.value)} placeholder="Degree" className="w-full p-2 border rounded-md" /><input value={entry.institution} onChange={e => handleEducationChange(index, 'institution', e.target.value)} placeholder="Institution" className="w-full p-2 border rounded-md" /><button type="button" onClick={() => removeEducationEntry(index)} className="absolute -top-2 -right-2 p-1 bg-destructive text-destructive-foreground rounded-full"><Trash2 size={14}/></button></div>))}<button type="button" onClick={addEducationEntry} className="flex items-center gap-2 text-sm font-semibold text-primary hover:underline"><Plus size={16}/> Add Education Entry</button></div>
                </div>
            </div>
