...
export default function MyMoneyPage() {
    ...
    useEffect(() => {
        const unsubscribers = [
            ...
            getMileage(TEMP_USER_ID, setMileageEntries),
    ...
    const allExpenses = useMemo(() => {
        const certExpenses: Expense[] = (certifications || [])
            ...
            .map(cert => ({ id: `cert-${cert.id}`, description: `Renewal for ${cert.name}`, amount: cert.renewalCost!, date: cert.issueDate || new Date().toISOString().split('T')[0], category: 'Professional Development', isReadOnly: true, }));

        const ceuExpenses: Expense[] = (allCeus || [])
            .filter(ceu => ceu.cost && ceu.cost > 0)
            .map(ceu => ({ id: `ceu-${ceu.id}`, description: `CEU: ${ceu.activityName}`, amount: ceu.cost!, date: ceu.dateCompleted, category: 'Professional Development', isReadOnly: true, }));
